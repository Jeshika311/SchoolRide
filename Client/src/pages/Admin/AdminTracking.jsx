  ...
  return (
    <SidebarLayout>
      <div className="max-w-6xl mx-auto space-y-6 text-slate-900">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Driver GPS Simulator</h1>
          <p className="text-sm text-slate-500">Simulate vehicle movements on maps and broadcast live Socket.io updates</p>
        </div>
        ...
        {loading ? (
          <div className="h-96 bg-white border border-sky-100 animate-pulse rounded-3xl shadow-sm" />
        ) : buses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            
            {/* Map Simulator */}
            <div className="md:col-span-2 bg-white p-3 rounded-3xl border border-sky-100 h-[440px] relative shadow-sm">
              <div 
                ref={mapContainerRef} 
                className="w-full h-full rounded-2xl z-10" 
                style={{ background: '#f8fafc' }}
              />
              <div className="absolute top-6 right-6 z-20 bg-white/90 backdrop-blur-md px-3.5 py-2 border border-sky-100 rounded-xl text-[10px] text-slate-500 font-medium shadow-sm">
                Click map to select coords
              </div>
            </div>

            {/* Simulation controls */}
            <div className="bg-white border border-sky-100 rounded-3xl p-6 space-y-6 shadow-sm">
              <h3 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Simulation controls</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-sky-100 rounded-xl py-2.5 px-3 text-xs text-slate-700 focus:outline-none focus:border-sky-400"
                  >
                    {buses.map(b => (
                ...
                <form onSubmit={handleTelemetrySubmit} className="space-y-4 pt-2 border-t border-sky-100">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-sky-100 rounded-xl py-2 px-3 text-xs text-slate-700 focus:outline-none"
                        required
                      />
                    </div>
                    <div>

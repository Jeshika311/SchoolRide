    ...
    <SidebarLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-white tracking-tight">Available Bus Commutes</h1>
            <p className="text-sm text-slate-400">Search and filter routes to find your school ride</p>
          </div>
          
          <button 
            onClick={handleClearFilters}
            className="text-xs font-semibold text-blue-400 hover:text-blue-300 bg-slate-950/20 px-3 py-2 rounded-lg border border-slate-800/80 w-fit self-end md:self-auto"
          >
            Clear All Filters
          </button>
        </div>
        ...
        {/* Filter Toolbar */}
        <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-950/40 p-4 rounded-2xl border border-slate-800/80">
          {/* Search bar */}
          ...
          <button 
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold py-2.5 rounded-xl transition-all duration-200"
          >
            Apply Filters
          </button>
        </form>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-64 bg-slate-950/20 border border-slate-800/80 animate-pulse rounded-2xl" />
            ))}
          </div>
        ) : buses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {buses.map((bus) => {
              const availableCount = bus.totalSeats - bus.occupiedSeats.length;
              return (
                <div 
                  key={bus._id}
                  className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-5 flex flex-col justify-between hover:border-slate-700/80 hover:scale-[1.01] transition-all duration-300"
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                        availableCount > 0 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
                      }`}>
                    ...
                    {/* Route Details */}
                    <div>
                      <h4 className="font-bold text-lg text-slate-100">{bus.routeName}</h4>
        ...
        {pagination.pages > 1 && (
          <div className="flex justify-center gap-2 mt-8">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 border border-slate-800 rounded-xl bg-slate-950/20 hover:bg-slate-900 text-slate-300 disabled:opacity-40 transition-colors"
            >
              Previous
            </button>
            <span className="flex items-center px-4 text-sm text-slate-400 font-medium">
